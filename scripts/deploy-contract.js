...
async function main() {
  ...
  try {
    ...
    // 更新constants.ts文件中的合约地址
    updateContractAddress(deployedContract.address);
...
function updateContractAddress(contractAddress) {
  try {
    const constantsFilePath = path.join(__dirname, "../lib/constants.ts");
    let constantsFile = fs.readFileSync(constantsFilePath, "utf8");
    
    // 替换合约地址
    constantsFile = constantsFile.replace(
      /export const ACHIEVEMENT_CONTRACT_ADDRESS = "0x[a-fA-F0-9]+"/,
      `export const ACHIEVEMENT_CONTRACT_ADDRESS = "${contractAddress}"`
    );
    
    // 保存更新后的文件
    fs.writeFileSync(constantsFilePath, constantsFile);
    console.log(`已更新合约地址到lib/constants.ts文件`);
  } catch (error) {
    console.error("更新constants.ts文件时发生错误:", error);
  }
}
