...
function parseDebugLevel(debug_level) {
  switch (debug_level) {
    case "0":
    case "1":
    case "2":
...
var _driverEnvHeader, _clientConfiguration, _httpClient, _lastTxnTs, _isClosed, _queryWithRetries, queryWithRetries_fn, _getError, getError_fn, _getSecret, getSecret_fn, _getEndpoint, getEndpoint_fn, _getLogger, getLogger_fn, _query, query_fn, _setHeaders, setHeaders_fn, _validateConfiguration, validateConfiguration_fn;
var _Client = class {
  constructor(clientConfiguration, httpClient) {
    ...
    __privateSet(this, _clientConfiguration, {
      ...
      logger: __privateMethod(this, _getLogger, getLogger_fn).call(this, clientConfiguration)
    });
  ...
  stream(tokenOrQuery, options) {
    ...
    if (isStreamClient(streamClient)) {
      const streamClientConfig = {
        ...
        logger: __privateGet(this, _clientConfiguration).logger,
        ...options
      };
...
_getLogger = new WeakSet();
getLogger_fn = function(partialClientConfig) {
  if (partialClientConfig && "logger" in partialClientConfig && partialClientConfig.logger === void 0) {
...
query_fn = async function(queryRequest, queryOptions, attempt = 0) {
  try {
    ...
    const method = "POST";
    __privateGet(this, _clientConfiguration).logger.debug(
    ...
    if (response.status === 200 && (response.body.length === 0 || response.headers["content-length"] === "0")) {
      throw new ProtocolError({
        message: "There was an issue communicating with Fauna. Response is empty. Please try again.",
        httpStatus: 500
      });
    }
    let parsedResponse;
...
Query result: ${JSON.stringify(maybeStreamToken, null)}`
  ...
  async *#startStream() {
    ...
    const streamAdapter = this.#clientConfiguration.httpStreamClient.stream(request);
...
var _FeedClient = class {
  ...
  async nextPage() {
    ...
    __privateGet(this, _clientConfiguration2).logger.debug(
      "Fauna HTTP %s request to '%s' (timeout: %s), headers: %s",
      request.method,
...
nextPageHttpRequest_fn = async function() {
  ...
  const client_timeout_ms = __privateGet(this, _clientConfiguration2).client_timeout_buffer_ms + __privateGet(this, _clientConfiguration2).query_timeout_ms;
  const method = "POST";
  const req = {
    headers,
    client_timeout_ms,
    method,
    data: { token: __privateGet(this, _eventSource).token },
